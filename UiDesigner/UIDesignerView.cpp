...
void CUIDesignerView::OnLButtonDown(UINT nFlags, CPoint point)
{
  ...
  CControlUI* pControl=m_LayoutManager.FindControl(point);
  ...
  int nHit=m_MultiTracker.HitTest(point);
  ...
  {
    m_MultiTracker.SetFocus(point);
  }

  if(nHit>=0||nType==typeControl)
  {
    m_MultiTracker.Track(this, point, FALSE,NULL);
  }
  else
  {
    CUITracker tracker;
    ...
    if (tracker.TrackRubberBand(this, point, TRUE))
    {
      ...
      {
        rect.SetRect(point.x,point.y,point.x+UI_DEFAULT_WIDTH,point.y+UI_DEFAULT_HEIGHT);
      }
    }
    else
    {
...
{
  // TODO: 在此添加消息处理程序代码和/或调用默认值
  if (pWnd==this&&m_MultiTracker.SetCursor(pWnd,nHitTest))
  {
    return TRUE;
  }
...
void CUIDesignerView::OnSize(UINT nType, int cx, int cy)
{
  ...
  SetScrollSizes(MM_TEXT,CSize(size.cx+80,size.cy+80));
}
