...
void swap(LPVOID pt0,LPVOID pt1,DWORD dwSize)
{
  BYTE  ch;
  DWORD i;
  ...
  {
    ch = ((PBYTE)pt0)[i];
    ((PBYTE)pt0)[i] = ((PBYTE)pt1)[i];
    ((PBYTE)pt1)[i] = ch;
  }
}

/***
*isortP(hi,lo,width,comp) - insertion sort for sorting short arrays
*
*Purpose:
...
*
*******************************************************************************/
void isortP(LPCVOID lo,LPCVOID hi,size_t width,int (* CompareP)(void *,const void *,const void *),void *context)
{
    PBYTE p,max;
    ...
    while (hi > lo)
    {
        ...
        max = (PBYTE)lo;
        for (p = (PBYTE)lo + width; p <= (PBYTE)hi; p += width)
        {
            /* A[i] <= A[max] for lo <= i < p */
            if (CompareP(context,p,max) > 0)
            {
                max = p;
            }
            /* A[i] <= A[max] for lo <= i <= p */
...
void qsort_s(LPCVOID base,size_t num,size_t width,int (* CompareP)(void *,const void *,const void *), void *context)
{
    PBYTE lo,hi;              /* ends of sub-array currently sorting */
    PBYTE mid;                  /* points to middle of subarray */
    PBYTE loguy,higuy;        /* traveling pointers for partition step */
    size_t size;              /* size of the sub-array */
    PBYTE lostk[30],histk[30];
    ...
    lo = (PBYTE)base;
    hi = (PBYTE)base + width * (num-1);        /* initialize limits */
...
recurse:
    ...
    if (size <= CUTOFF)
    {
         isortP(lo,hi,width,CompareP,context);
    }
    else
    {
        ...
        for (;;)
        {
            ...
            {
                loguy += width;
            } while (loguy <= hi && CompareP(context,loguy,lo) <= 0);
            ...
            {
                higuy -= width;
            } while (higuy > lo && CompareP(context,higuy,lo) >= 0);
